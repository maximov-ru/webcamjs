...
var Webcam = {
	...
	attach: function(elem) {
		...
		if (!this.params.dest_width && this.params.width) this.params.dest_width = this.params.width;
		if (!this.params.dest_height && this.params.height) this.params.dest_height = this.params.height;
		...
		var scaleX = this.params.width ? this.params.width / this.params.dest_width : 1;
		var scaleY = this.params.height ? this.params.height / this.params.dest_height : 1;
		...
		if (this.userMedia) {
			...
			if (this.params.dest_width) {
				mandatory.minWidth = this.params.dest_width;
			}

			if (this.params.dest_height) {
				mandatory.minHeight = this.params.dest_height;
			}

			if (!mandatory.minWidth && this.params.display_aspect_ratio) {
				mandatory.minWidth = mandatory.minHeight * this.params.display_aspect_ratio;
			}

			if (!mandatory.minHeight && this.params.display_aspect_ratio) {
			...
			if (!this.params.width && this.params.display_aspect_ratio) {
				div.style.width = '' + (this.params.height * this.params.display_aspect_ratio) + 'px';
			}

			if (!this.params.height && this.params.display_aspect_ratio) {
				div.style.height = '' + (this.params.width / this.params.display_aspect_ratio) + 'px';
			}
			...
			if (!this.params.dest_width) {
				img.style.width = '' + (this.params.dest_height * this.params.display_aspect_ratio) + 'px';
			}

			if (!this.params.dest_height) {
				img.style.height = '' + (this.params.dest_width / this.params.display_aspect_ratio) + 'px';
			}
			...
			input.addEventListener('change', function(event) {
				if (event.target.files.length > 0 && event.target.files[0].type.indexOf('image/') == 0) {
					...
					image.addEventListener('load', function(event) {
						...
						//calculating sizes
						var ratio = 1;
						if (params.dest_width) {
							var widthRatio = image.width / params.dest_width;
							ratio = widthRatio;
						}

						if (params.dest_height) {
							var heightRatio = image.height / params.dest_height;
							ratio = heightRatio;
						}

						if (params.dest_width && params.dest_height) {
	...
	snap: function(user_callback, user_canvas) {
		...
		//calc dest_width && dest_height
		if (this.video) {
			if (this.params.dest_width) {
				var dest_width = this.params.dest_width;
				var widthRatio = this.video.videoWidth / this.params.dest_width;
			}

			if (this.params.dest_height) {
				var dest_height = this.params.dest_height;
				var heightRatio = this.video.videoHeight / this.params.dest_height;
			}

			if (!this.params.dest_width) {
				var dest_width = this.video.videoWidth / heightRatio;
			}

			if (!this.params.dest_height) {
				var dest_height = this.video.videoHeight / widthRatio;
			}
		} else {
			if (this.params.dest_width) {
				var dest_width = this.params.dest_width;
			...
			if (!this.params.dest_width && this.params.display_aspect_ratio) {
				var dest_width = dest_height * this.params.display_aspect_ratio;
			}

			if (!this.params.dest_height) {
				var dest_height = dest_width / this.params.display_aspect_ratio;
			}
		}
		...
		var func = function() {
			// render image if needed (flash) (iphone)
			...
			if (params.crop_width && params.crop_height) {
				...
				crop_context.drawImage( canvas,
					Math.floor( (dest_width / 2) - (params.crop_width / 2) ),
					Math.floor( (dest_height / 2) - (params.crop_height / 2) ),
					params.crop_width,
					params.crop_height,
					0,
					0,
		...
		if (this.userMedia) {
			...
			context.drawImage(videoCanvas, 0, 0, dest_width, dest_height);
